...
public struct ConversionHomeView: View {
    ...
    @State private var cardAnimationGeneration = 0
    ...
    public var body: some View {
        NavigationStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    LazyVStack(spacing: 14) {
                        ForEach(
                            ...
                            .buttonStyle(ConversionHomeCardButtonStyle())
                            .modifier(
            ...
            .onAppear {
                animateCardsIfNeeded()
                Task {
                    recentRecords = await ConversionHomeHistoryLoader.shared.load()
                }
            }
    ...
    private func animateCardsIfNeeded() {
        guard isTabActive else { return }
        cardAnimationGeneration &+= 1
        let generation = cardAnimationGeneration
        resetVisibleCardsWithoutAnimation()

        guard !reduceMotion else {
            visibleCardCount = ConversionHomeKind.allCases.count
            return
        }

        Task { @MainActor in
            await Task.yield()
            guard isTabActive, generation == cardAnimationGeneration else { return }

            for index in ConversionHomeKind.allCases.indices {
                if index > 0 {
    ...
    private func resetVisibleCardsWithoutAnimation() {
...
private enum ConversionHomeKind: String, CaseIterable, Identifiable, Hashable, Sendable {
    ...
    var title: String {
        switch self {
        case .image: L10n.string("conversion.home.image.title")
        case .video: L10n.string("conversion.home.video.title")
        case .audio: L10n.string("conversion.home.audio.title")
        }
    }

    var detail: String {
        switch self {
        case .image: L10n.string("conversion.home.image.tagline")
        case .video: L10n.string("conversion.home.video.tagline")
        case .audio: L10n.string("conversion.home.audio.tagline")
        }
    }
    ...
    func load() async -> [ConversionHomeKind: ConversionHomeRecord] {
        ...
        for kind in ConversionHomeKind.allCases {
            ...
            let completed = records.compactMap { record -> (URL, Date)? in
                guard record.status == .completed, let outputPath = record.outputPath else {
                    return nil
                }
                ...
                let values = try? outputURL.resourceValues(forKeys: [.contentModificationDateKey])
                return (outputURL, values?.contentModificationDate ?? .distantPast)
            }

            guard let latest = completed.max(by: { $0.1 < $1.1 }) else { continue }
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Label(
                        L10n.format(
                            ...
                            recentRecord.completedAt.formatted(.relative(presentation: .named)),
...
private struct ConversionHomeStaggeredAppear: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .opacity(isVisible ? 1 : (reduceMotion ? 1 : 0.035))
            .offset(y: isVisible || reduceMotion ? 0 : 320)
    }
}
