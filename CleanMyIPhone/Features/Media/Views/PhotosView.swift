...
private struct MediaCategoryDetailView: View {
    ...
    /// 封装 `assetButton` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    /// 封装 `dateTitle` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    /// 封装 `toggleSelection` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    @ViewBuilder
    /// 封装 `livePhotoBadge` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    @ViewBuilder
    /// 封装 `videoDurationBadge` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    /// 封装 `videoDurationText` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    /// 封装 `assetInformation` 对应的局部行为，供当前类型在统一入口下复用。
    ...
    @ViewBuilder
    /// 处理 `selectionIndicator` 导入流程，并将用户选择安全地交给后续处理。
